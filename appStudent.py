...
STUDENT_DATA_DIR = os.path.join(BASE_DIR, 'student_records')
STUDENT_UPLOAD_DIR = os.path.join(STUDENT_DATA_DIR, 'uploads')
ALLOWED_UPLOAD_EXTENSIONS = {'.png', '.jpg', '.jpeg', '.gif', '.heic', '.heif', '.pdf'}
...
def _ensure_student_dirs(bank_id):
    os.makedirs(STUDENT_DATA_DIR, exist_ok=True)
    bank_dir = os.path.join(STUDENT_DATA_DIR, bank_id)
    os.makedirs(bank_dir, exist_ok=True)
    os.makedirs(os.path.join(STUDENT_UPLOAD_DIR, bank_id), exist_ok=True)
    return bank_dir
...
def _store_student_response(bank_id, record, uploaded_file):
    bank_dir = _ensure_student_dirs(bank_id)
    attachment_path = None

    if uploaded_file and uploaded_file.filename:
        original_name = secure_filename(uploaded_file.filename)
        if not _is_allowed_upload(original_name):
            abort(400, description="Unsupported file type for upload.")

        _, ext = os.path.splitext(original_name)
        storage_name = f"{record['session_id']}_{record['question_id']}_{uuid.uuid4().hex}{ext.lower()}"
        bank_upload_dir = os.path.join(STUDENT_UPLOAD_DIR, bank_id)
        os.makedirs(bank_upload_dir, exist_ok=True)
        storage_path = os.path.join(bank_upload_dir, storage_name)
        uploaded_file.save(storage_path)
        attachment_path = os.path.relpath(storage_path, BASE_DIR)

    record_payload = dict(record)
    record_payload['attachment_path'] = attachment_path
    record_payload['timestamp'] = datetime.utcnow().isoformat() + 'Z'
...
def practice_session(bank_id):
    ...
    try:
        requested_groups = int(request.args.get('count', 10))
    except (TypeError, ValueError):
        requested_groups = 10
    requested_groups = max(1, requested_groups)
    ...
    for topic_name, groups in questions_data.items():
        ...
        for group in groups:
            ...
            if not parts:
                continue
            parts.sort(key=lambda item: _natural_key(item['id']))
            grouped_questions.append({
                'group_id': group.get('group_id') or parts[0]['id'],
                'topic': topic_name,
                'parts': parts,
            })
    ...
    selected_groups = grouped_questions[:requested_groups]
    ...
    for group_index, group in enumerate(selected_groups, start=1):
        ...
        for part_index, part in enumerate(group['parts'], start=1):
            ...
            item['part_number'] = part_index
            item['part_count'] = part_count
            session_questions.append(item)

    return jsonify(session_questions)


@app.route('/practice/<bank_id>/response', methods=['POST'])
def log_practice_response(bank_id):
    ...
    if not question_id:
        return jsonify({'success': False, 'error': 'Missing question identifier.'}), 400

    allowed_ratings = {'correct', 'partial', 'wrong', 'skip'}
    if rating not in allowed_ratings:
        return jsonify({'success': False, 'error': 'Unknown rating supplied.'}), 400
    ...
    record = {
        ...
        'group_id': _normalize(form.get('group_id') or ''),
        'topic': _normalize(form.get('topic') or ''),
        'rating': rating,
        'answer_text': _normalize(form.get('answer_text') or ''),
    }

    # Numeric metadata is optional; store as ints if possible
    for key in ('group_index', 'part_number', 'part_count'):
        value = form.get(key)
    ...
    if points not in (None, ''):
        try:
            record['points'] = float(points)
        except (TypeError, ValueError):
            record['points'] = points

    stored = _store_student_response(bank_id, record, uploaded_file)

    return jsonify({'success': True, 'recorded': stored})


@app.route('/practice/<bank_id>/responses')
def list_practice_responses(bank_id):
    get_bank_config(bank_id)
    ...
    if os.path.isdir(bank_dir):
        for filename in sorted(os.listdir(bank_dir), reverse=True):
                ...
                with open(file_path, 'r', encoding='utf-8') as handle:
                    for line in reversed(handle.readlines()):
                        ...
                        try:
                            ...
                            continue
                        if student_filter and entry.get('student_id') != student_filter:
                            continue
    ...
    return jsonify({'bank_id': bank_id, 'responses': responses})
